    ...
    private class GestureListener extends GestureDetector.SimpleOnGestureListener {
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            if (Math.abs(e1.getX() - e2.getX()) > Math.abs(e1.getY() - e2.getY())) {
                // horizontal swipe
                if (e2.getX() > e1.getX())
                    m_controller.swipe(getContext(), GameController.D_RIGHT);
                else
                    m_controller.swipe(getContext(), GameController.D_LEFT);
            } else {
                // vertical swipe
                if (e2.getY() > e1.getY())
                    m_controller.swipe(getContext(), GameController.D_DOWN);
                else
                    m_controller.swipe(getContext(), GameController.D_UP);
            }
